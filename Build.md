...
_BDiff / BPatch_ are written in Object Pascal and are designed to be compiled with Delphi 13 or later. The programs can be built entirely from within the Delphi IDE, provided that some [prerequisites](#prerequisites) are met.

> ⛔ Versions of Delphi prior to Delphi 13 will not compile the code.

Both _BDiff_ and _BPatch_ can be compiled as either 32 or 64 bit Windows programs.
...
* `BDSBIN` must be set to `%BDS%\bin`. (`BDS` will have been set when `rsvars.bat` was run.)
...
All binary output from the compilation will be written to the `_build` directory. Once _BDiff_ and _BPatch_ have both been built for both Windows 32-bit and Windows 64-bit target platforms, you will see the following sub-directories under `_build`:
